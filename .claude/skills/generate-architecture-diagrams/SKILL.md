...
/generate-architecture-diagrams --architecture=architecture/platform-3.3.0/component-a.md
# → Outputs to: architecture/platform-3.3.0/diagrams/

# Specify custom output directory
/generate-architecture-diagrams --architecture=./GENERATED_ARCHITECTURE.md --output-dir=./custom-diagrams

# Generate specific formats only
/generate-architecture-diagrams --architecture=architecture/platform-3.3.0/component-a.md --formats=mermaid,security
```
...
**Examples**:
- `architecture/platform-3.3.0/component-a.md` → `component-a`
- `architecture/platform-3.3.0/PLATFORM.md` → `platform`
...
- Architecture file: `architecture/platform-3.3.0/component-a.md`
- Output directory: `architecture/platform-3.3.0/diagrams/`
...
**Examples**:
- `architecture/platform-3.3.0/component-a.md` → `architecture/platform-3.3.0/diagrams/`
...
**Examples**:
- `component-a.md` → `component-a-component.mmd`, `component-a-dataflow.mmd`, etc.
...
```mermaid
graph TB
    subgraph "ComponentB Component"
        Controller[ComponentB Controller]
    ...
    Controller -->|creates| ServerlessEngineService[ServerlessEngine Service]
    Controller -->|creates| Deployment[Predictor Deployment]

    Controller -->|depends on| ServiceMesh[ServiceMesh]
    Controller -->|depends on| ServerlessEngine[ServerlessEngine Serving]
    Controller -->|integrates| ModelRegistry[ComponentC]

    style Controller fill:#4a90e2
    style Webhook fill:#4a90e2
    style InferenceServiceCR fill:#f5a623
    style ServiceMesh fill:#e8e8e8
    style ServerlessEngine fill:#e8e8e8
...
sequenceDiagram
    ...
    participant Gateway as ServiceMesh Gateway<br/>443/TCP HTTPS TLS1.3
    participant Activator as ServerlessEngine Activator<br/>8012/TCP mTLS
...
│         │ Auth: Service Identity (ServiceMesh)                                 │
│         ▼                                                                │
├─────────────────────────────────────────────────────────────────────────┤
│  SERVICE MESH (Trusted - mTLS STRICT)                                   │
│                                                                          │
│  [ServerlessEngine Activator]                                                     │
...
graph TB
    ...
    subgraph Ingress["INGRESS (DMZ)"]
        Gateway["ServiceMesh Gateway<br/>443/TCP → 8080/TCP<br/>TLS 1.3<br/>RequestAuthentication"]
    end
    ...
    subgraph ServiceMesh["SERVICE MESH (mTLS STRICT)"]
        ...
        subgraph NS_ServerlessEngine["Namespace: knative-serving"]
            Activator["ServerlessEngine Activator<br/>SA: activator"]
        end
...
workspace {
    model {
        ...
        kserve = softwareSystem "ComponentB" "Standardized serverless ML inference platform" {
            controller = container "ComponentB Controller" "Manages InferenceService lifecycle" "Go Operator"
            webhook = container "Webhook Server" "Validates and mutates InferenceServices" "Go Service"
        }

        istio = softwareSystem "ServiceMesh" "Service mesh for traffic management" "External"
        knative = softwareSystem "ServerlessEngine Serving" "Serverless autoscaling platform" "External"
        modelRegistry = softwareSystem "ComponentC" "Stores metadata" "Internal Platform"
...
```mermaid
graph LR
    ComponentB[ComponentB Operator]

    %% External Dependencies
    ComponentB -->|traffic mgmt| ServiceMesh[ServiceMesh 1.20+]
    ComponentB -->|autoscaling| ServerlessEngine[ServerlessEngine Serving 1.12+]
    ComponentB -->|certificates| CertManager[cert-manager 1.13+]

    %% Internal Platform Dependencies
    ComponentB -->|API calls| ModelRegistry[ComponentC]
    ComponentB -.->|sidecars| ServiceMesh[Service Mesh]

    %% Integration Points
    Dashboard[Dashboard] -->|UI mgmt| ComponentB
    Pipelines[Data Science Pipelines] -->|auto deploy| ComponentB

    %% External Services
    ComponentB -->|model storage| S3[S3 Storage]
    ComponentB -->|API server| K8sAPI[Kubernetes API]

    style ComponentB fill:#4a90e2,stroke:#333,stroke-width:4px
    style ServiceMesh fill:#999,stroke:#333
    style ServerlessEngine fill:#999,stroke:#333
...
Width: 3000px, Chrome: /usr/bin/google-chrome
  ...
  component-a-component.mmd → component-a-component.png
  component-a-dataflow.mmd → component-a-dataflow.png
  component-a-security-network.mmd → component-a-security-network.png
  component-a-dependencies.mmd → component-a-dependencies.png
  component-a-rbac.mmd → component-a-rbac.png
...
- Example: `architecture/platform-3.3.0/component-a.md` → `architecture/platform-3.3.0/diagrams/`
...
```
architecture/platform-3.3.0/                # ← Directory is versioned
├── component-a.md                           # Component architecture
├── kserve.md
├── model-registry.md
├── diagrams/                          # Auto-created (shared by all components)
│   ├── component-a-component.mmd            # ← No version in filename (redundant)
│   ├── component-a-component.png            # ← Auto-generated PNG (3000px width)
│   ├── component-a-dataflow.mmd
│   ├── component-a-dataflow.png
│   ├── component-a-security-network.mmd     # Mermaid (visual, editable)
│   ├── component-a-security-network.png     # PNG (high-res)
│   ├── component-a-security-network.txt     # ASCII (precise)
│   ├── component-a-c4-context.dsl
│   ├── component-a-dependencies.mmd
│   ├── component-a-dependencies.png
│   ├── component-a-rbac.mmd
│   ├── component-a-rbac.png
...
**Key principles**:
- Filenames use base component name only (no version). The directory `platform-3.3.0/` provides versioning context.
...
```bash
# Auto output to architecture/platform-3.3.0/diagrams/
/generate-architecture-diagrams --architecture=architecture/platform-3.3.0/component-a.md
...
/generate-architecture-diagrams --architecture=architecture/platform-3.3.0/component-a.md --output-dir=./component-a-diagrams
```

### Optimize for Audience

```bash
/generate-architecture-diagrams --architecture=architecture/platform-3.3.0/component-a.md --audience=security  # Emphasizes security details
/generate-architecture-diagrams --architecture=architecture/platform-3.3.0/component-a.md --audience=executive # Simplified, high-level views
```
...
/repo-to-architecture-summary checkouts/platform-org/component-a
