...
/generate-platform-diagrams --platform-file=architecture/platform-3.3.0/PLATFORM.md
...
Example: If platform file is `architecture/platform-3.3.0/PLATFORM.md`, output to `architecture/platform-3.3.0/diagrams/`
...
graph TB
    subgraph "Core Platform Services"
        ServiceMesh[ServiceMesh<br/>Service Mesh]
        ServerlessEngine[ServerlessEngine Serving<br/>Serverless]
    ...
    %% Core dependencies
    ComponentB -->|traffic mgmt| ServiceMesh
    ComponentB -->|autoscaling| ServerlessEngine
    ComponentB -->|TLS certs| CertManager

    %% Component integrations
    Dashboard -->|UI management| ComponentB
    Dashboard -->|displays models| ModelRegistry
    Pipelines -->|auto-deploy| ComponentB
    Pipelines -->|stores models| ModelRegistry
    ComponentB -->|model metadata| ModelRegistry

    %% External dependencies
    ComponentB -->|model storage| S3
    ModelRegistry -->|artifact storage| S3
    Dashboard -->|API calls| K8sAPI

    %% Styling
    style ServiceMesh fill:#e8e8e8,stroke:#333
    style ServerlessEngine fill:#e8e8e8,stroke:#333
    style CertManager fill:#e8e8e8,stroke:#333
    style ComponentB fill:#4a90e2,stroke:#333,stroke-width:3px
...
**Instructions**:
- Group components by category (Core Infrastructure, Platform Components, External)
- Show dependency arrows with labels (e.g., "traffic mgmt", "data access")
- Highlight central components with thicker borders
- Use consistent color scheme: gray for external platform, blue for key components, green for platform components, orange for external services
...
│  INGRESS│(DMZ)                                                          │
│         │                                                                │
│         ├─→ [Dashboard - dashboard-platform-ns.apps.cluster.com]    │
│         │   Port: 443/TCP (ext) → 8080/TCP (int)                        │
│         │   Protocol: HTTPS, Encryption: TLS 1.3                         │
│         │   Auth: OAuth (optional)                                       │
│         │                                                                │
│         ├─→ [ComponentB Gateway - *.kserve.apps.cluster.com]                │
...
│         │   ├─→ ComponentC (gRPC/9090, mTLS)                        │
│         │   ├─→ ComponentB (HTTP/8080, mTLS)                                │
│         │   └─→ Kubernetes API (HTTPS/443, ServiceAccount token)        │
│         │                                                                │
│         ├─→ [ComponentB Controller]                                          │
│         │   Namespace: platform-ns                                       │
│         │   ServiceAccount: kserve-controller-manager                    │
│         │   ├─→ ComponentC (gRPC/9090, mTLS)                        │
│         │   ├─→ ServerlessEngine Serving (HTTP/8080, mTLS)                       │
│         │   ├─→ ServiceMesh (HTTP/8080, mTLS)                                 │
│         │   └─→ Kubernetes API (HTTPS/443, ServiceAccount token)        │
│         │                                                                │
│         ├─→ [ComponentC]                                             │
│         │   Namespace: platform-ns                                       │
...
│  - Encryption: TLS 1.2+                                                  │
│  - Components: ComponentB, ComponentC, Pipelines                         │
│                                                                          │
│  [Container Registry - quay.io]                                          │
│  - Purpose: Pull runtime images                                          │
│  - Auth: Pull secrets                                                    │
│  - Encryption: TLS 1.2+                                                  │
│  - Components: ComponentB, Workloads                                       │
└─────────────────────────────────────────────────────────────────────────┘

NAMESPACES:
- platform-ns: Main platform namespace (mTLS STRICT)
- knative-serving: ServerlessEngine components (mTLS PERMISSIVE)
- istio-system: Service mesh control plane

SERVICE MESH CONFIGURATION:
- Mode: STRICT mTLS for platform-ns namespace
...
- model-registry-role: modelregistry.platform-ns.io/*
- odh-dashboard-role: platform-ns.io/*, monitoring.coreos.com/servicemonitors
```
...
graph TB
    ...
    subgraph Ingress["INGRESS (DMZ)"]
        Dashboard_GW["Dashboard Gateway<br/>443/TCP → 8080/TCP<br/>HTTPS, TLS 1.3<br/>OAuth (optional)"]
        ComponentB_GW["ComponentB Gateway<br/>443/TCP → 8012/TCP<br/>HTTPS, TLS 1.3<br/>Bearer Token (optional)"]
    end

    subgraph ServiceMesh["SERVICE MESH (mTLS STRICT)"]
        direction TB

        subgraph NS_Platform["Namespace: platform-ns"]
            Dashboard["Dashboard<br/>SA: odh-dashboard"]
            ComponentB["ComponentB Controller<br/>SA: kserve-controller-manager"]
            Registry["ComponentC<br/>SA: model-registry"]
            Pipelines["DS Pipelines<br/>SA: pipeline-runner"]
        end

        subgraph NS_ServerlessEngine["Namespace: knative-serving"]
            ServerlessEngine["ServerlessEngine Serving"]
        end

        subgraph NS_ServiceMesh["Namespace: istio-system"]
            ServiceMesh["ServiceMesh Control Plane"]
        end
    end
    ...
    Clients -->|HTTPS/443| ComponentB_GW

    %% Ingress to Service Mesh
    Dashboard_GW -->|HTTP/8080<br/>mTLS| Dashboard
    ComponentB_GW -->|HTTP/8012<br/>mTLS| ComponentB

    %% Service Mesh Internal
    Dashboard -->|gRPC/9090<br/>mTLS| Registry
    Dashboard -->|HTTP/8080<br/>mTLS| ComponentB
    Dashboard -->|HTTPS/443<br/>SA token| K8sAPI

    ComponentB -->|gRPC/9090<br/>mTLS| Registry
    ComponentB -->|HTTP/8080<br/>mTLS| ServerlessEngine
    ComponentB -->|HTTP/8080<br/>mTLS| ServiceMesh
    ComponentB -->|HTTPS/443<br/>SA token| K8sAPI

    Pipelines -->|HTTP/8080<br/>mTLS| ComponentB
    ...
    ComponentB -->|HTTPS/443<br/>AWS IAM| S3
    Pipelines -->|HTTPS/443<br/>AWS IAM| S3
    ComponentB -->|HTTPS/443<br/>Pull secret| Quay
    ...
    class Dashboard_GW,ComponentB_GW ingress
    class Dashboard,ComponentB,Registry,Pipelines,ServerlessEngine,ServiceMesh servicemesh
...
sequenceDiagram
    ...
    participant ComponentB as ComponentB
    participant Registry as ComponentC
    participant Gateway as ServiceMesh Gateway
    ...
    Pipelines->>ComponentB: Create InferenceService CR
    ComponentB->>Registry: Fetch model location<br/>(gRPC, mTLS)
    Registry-->>ComponentB: S3 path + credentials
    ComponentB->>ComponentB: Create predictor pod
    Note over ComponentB: Pod pulls image from quay.io
    ComponentB->>S3: Download model artifacts<br/>(AWS IAM)
    ComponentB->>Gateway: Configure routing<br/>(VirtualService)

    %% Production inference
    Client->>Gateway: POST /v1/models/my-model:predict<br/>(HTTPS, TLS 1.3)
    Gateway->>ComponentB: Forward request<br/>(mTLS)
    ComponentB->>ComponentB: Run inference
    ComponentB-->>Gateway: Prediction result<br/>(mTLS)
    Gateway-->>Client: JSON response<br/>(HTTPS)

    %% Monitoring
    User->>Dashboard: View model metrics
    Dashboard->>ComponentB: Query metrics<br/>(K8s API)
...
graph TD
    subgraph "Authentication Mechanisms"
        OAuth[OAuth/OIDC<br/>Dashboard]
        Bearer[Bearer Tokens<br/>ComponentB Inference]
    ...
    subgraph "Key ServiceAccounts"
        SA1[kserve-controller-manager<br/>Namespace: platform-ns]
        SA2[odh-dashboard<br/>Namespace: platform-ns]
        SA3[model-registry<br/>Namespace: platform-ns]
        SA4[pipeline-runner<br/>Namespace: platform-ns]
    end

    subgraph "ClusterRoles (Top Privileged)"
        CR1[kserve-manager-role<br/>serving.kserve.io/*<br/>core/services, apps/deployments]
        CR2[odh-dashboard-role<br/>platform-ns.io/*<br/>monitoring/servicemonitors]
        CR3[model-registry-role<br/>modelregistry.platform-ns.io/*]
    end
    ...
    subgraph "Service Mesh Security"
        PA[PeerAuthentication<br/>Mode: STRICT<br/>Namespace: platform-ns]
...
graph LR
    subgraph "Platform Metrics - MyPlatform 3.3.0"
        direction TB
        A[Total Components: 12]
        B[Operator-based: 8 of 12 = 67%]
        C[Service Mesh: mTLS STRICT]
        D[CRD API Versions: v1]
        E[External Dependencies: 3<br/>ServiceMesh, ServerlessEngine, cert-manager]
...
```
================================================================================
PLATFORM MATURITY DASHBOARD - MyPlatform 3.3.0
================================================================================
...
```bash
/generate-platform-diagrams --platform-file=architecture/platform-2.19/PLATFORM.md
```
