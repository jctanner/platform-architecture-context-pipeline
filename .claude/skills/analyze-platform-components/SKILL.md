...
description: Analyze all components for a platform sequentially by parsing component manifest or discovery script and executing repo-to-architecture-summary for each. Skips components that already have GENERATED_ARCHITECTURE.md. Fully autonomous and resumable.
...
Automatically discover and analyze all components for a platform by:
1. Parsing component manifest script (e.g., `get_all_manifests.sh`) from the operator repository to get the authoritative component list
...
Required:
- `--platform=name` - Platform to analyze

Optional:
- `--checkouts-dir=<path>` (default: ./checkouts)
- `--filter=<pattern>` - Only analyze components matching pattern (e.g., "component-a", "comp*")

Examples:
```bash
# Analyze all platform components sequentially
/analyze-platform-components --platform=myplatform

# Analyze specific components only
/analyze-platform-components --platform=myplatform --filter=component-a
/analyze-platform-components --platform=myplatform --filter="comp*"
```
...
{
  "kserve": {
    "repo_org": "platform-org",
    "repo_name": "kserve",
    "checkout_path": "checkouts/platform-org/kserve",
    "has_architecture": false
  },
  "dashboard": {
    "repo_org": "platform-org",
    "repo_name": "odh-dashboard",
    "checkout_path": "checkouts/platform-org/odh-dashboard",
...
Check platform-specific organization or checkout structure.
```
...
- Platform: Auto-detect from checkout path or use `--platform` value
...
2. Prepare repository (run `make get-manifests` for platform operator if needed)
...
```bash
# 1. Analyze all platform components (sequential, resumable)
/analyze-platform-components --platform=myplatform
...
/aggregate-platform-architecture --platform=myplatform --version=3.3.0
...
# Wait for completion...
/repo-to-architecture-summary checkouts/platform-org/component-b
...
# Analyzes all N components sequentially
