...
- `--platform=name` (required or auto-detect)
- `--version=X.Y` (required or auto-detect)
...
/aggregate-platform-architecture --platform=myplatform --version=3.3

# Custom architecture directory
/aggregate-platform-architecture --platform=myplatform --version=2.19 --architecture-dir=./docs/arch

# Legacy: search checkout repos directly
/aggregate-platform-architecture --platform=myplatform --version=3.3 --repos-dir=./checkouts/platform-org
```
...
ARCH_DIR={architecture-dir}/{platform}-{version}
if [ -d "$ARCH_DIR" ]; then
...
**Examples**:
- With `--platform=myplatform --version=3.3` → looks in `./architecture/myplatform-3.3/`
- With `--platform=myplatform --version=2.19 --architecture-dir=./docs` → looks in `./docs/myplatform-2.19/`
- With `--repos-dir=./checkouts/platform-org` → searches checkouts (legacy mode)
...
**If using legacy repos-dir**: Filter components by platform metadata:
- Filter based on `--platform` value
- Include components with matching "Platform:" metadata field
- Include components that list multiple platforms if the specified platform is among them

### Step 4: Extract Structured Data

From each component markdown, parse:
- **Dependencies**: Look for "### Internal Platform Dependencies" tables
...
- **Version**: {version}
- **Release Date**: {date}
- **Base Platform**: Kubernetes {version}+ (or specific distribution)
...
Example:
```
component-a → component-c (API calls)
              → component-b (API calls)

component-b → istio (traffic management)
       → knative-serving (autoscaling)
       → component-c (model metadata)

component-d → component-b (model deployment)
                       → component-c (model storage)
```
...
- Location: `{architecture-dir}/{platform}-{version}/PLATFORM.md`
- Example: `./architecture/myplatform-3.3/PLATFORM.md`

**If using legacy repos-dir**:
- Filename: `{platform}-{version}-PLATFORM.md`
- Location: `platform-architecture/{platform}-{version}-PLATFORM.md`
- Example: `platform-architecture/myplatform-3.3-PLATFORM.md`
...
**Example outputs**:
- Using organized directory: `./architecture/myplatform-3.3/PLATFORM.md`
- Using legacy mode: `platform-architecture/myplatform-3.3-PLATFORM.md`
...
3. **Aggregate**: Run this skill with `--platform` and `--version` to create platform-level view
...
- **Legacy repos-dir mode**: Filters components by Platform metadata from markdown
