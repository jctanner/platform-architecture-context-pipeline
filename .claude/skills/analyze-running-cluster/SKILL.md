...
description: Validate architecture documentation against a running Kubernetes cluster. Compares expected architecture (from GENERATED_ARCHITECTURE.md) with actual deployed resources to find drift.
...
Compare generated architecture documentation against an actual running Kubernetes cluster to identify drift between documentation and reality.
...
Example: `/analyze-running-cluster --kubeconfig=~/.kube/cluster-config --architecture=./GENERATED_ARCHITECTURE.md`
...
Use `kubectl` (or `oc` for OpenShift) to query the cluster:
...
# Get Service Mesh PeerAuthentication (if service mesh present)
kubectl get peerauthentication -n {namespace} -o json

# Get Service Mesh AuthorizationPolicy (if service mesh present)
...
/analyze-running-cluster --architecture=kserve/GENERATED_ARCHITECTURE.md --namespace=platform-ns
```
