...
description: Auto-discover all components of a platform, clone repositories, analyze each one, and generate platform architecture. Automates the full analysis workflow.
...
Automatically discover which repositories are part of a platform, clone them, analyze each component, and aggregate into a platform architecture document.
...
- `--org=<github-org>` (required - GitHub organization name)
- `--platform=name` (required - platform identifier)
- `--version=X.Y` (default: auto-detect)
- `--branch=<branch>` (default: main or platform-specific)
- `--output-dir=<path>` (default: ./platform-analysis)

Example: `/discover-platform-components --org=myorg --platform=myplatform --version=3.3`
...
- **Branch**: `--branch` value or default to `main`
- **Platform**: `--platform` value for identification
...
- Include repos likely to be components: operators, services, controllers
- Look for platform-specific keywords in repository names/descriptions
- Exclude: documentation, tests, tooling, ci-cd repos

Use the platform operator repository's configuration to find the canonical list of components:
1. Clone `{org}/{platform-operator-repo}` (typically named `{platform}-operator` or similar)
2. Check for component lists in:
   - `config/` directory
   - `components/` directory
   - `manifests/` directory
   - Any YAML/script files listing components
...
/repo-to-architecture-summary --platform={platform} --version={version}
```
...
/aggregate-platform-architecture --platform={platform} --version={version} --repos-dir={output-dir}/repos
```

This creates `{platform}-{version}-PLATFORM.md` in `{output-dir}/platform-architecture/`
...
Output directory: {output-dir}/
  repos/                  ({component-count} component repos)
    component-a/GENERATED_ARCHITECTURE.md
    component-b/GENERATED_ARCHITECTURE.md
    ...
  platform-architecture/
    {distribution}-{version}-PLATFORM.md
  DISCOVERY_REPORT.md
...
- For private platforms, may need access to private organization repositories
