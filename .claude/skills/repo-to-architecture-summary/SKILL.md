...
description: Analyze a component repository and generate comprehensive architecture summary with structured markdown tables. Use when analyzing platform components, documenting architecture, or creating security diagrams.
allowed-tools: Read, Glob, Grep, Bash(git *), Bash(ls *), Bash(find *), Write
---

# Repo to Architecture Summary

Analyze a component repository for a platform and generate a `GENERATED_ARCHITECTURE.md` file with detailed architecture information.
...
Optional flags:
- `--platform=name` (default: auto-detect or none)
- `--version=X.Y` (default: auto-detect)

Examples:
```bash
/repo-to-architecture-summary                                    # analyze current directory
/repo-to-architecture-summary checkouts/platform-org/component   # analyze specific directory
/repo-to-architecture-summary ./component --platform=myplatform --version=3.3
```
...
**For platform operator repositories**:
Check for setup targets in Makefile:
```bash
make get-manifests   # or similar targets
```
- Some operators pull external manifests/dependencies during build
- These may include component deployment configurations
- Check for `./opt/`, `./manifests/`, or similar generated directories
- Critical for analyzing actual deployment configuration (not just operator code)

**For operators with gateway controllers**:
- Check `internal/controller/services/gateway/` or similar paths for ingress controller code
- Some operators deploy platform ingress infrastructure:
  - Gateway API (gateway.networking.k8s.io)
  - Ingress Controllers
  - Auth proxies (oauth-proxy, kube-rbac-proxy, etc.)
  - Service mesh integration
- Document this in the Ingress section even though it's not in static manifests
- This is critical platform ingress mechanism
...
2. Languages and frameworks:
   ...
   - Containers: **PRIORITIZE `Dockerfile.konflux` or `Containerfile.konflux`** if they exist
     - Some platforms use Konflux or similar build systems
...
3. Deployment artifacts:
   ...
   - **For platform operators**: Check `./opt/`, `./manifests/`, or similar directories for pulled manifests (may be populated by `make get-manifests` or similar)
...
# Gateway API usage (modern ingress pattern)
...
- `openshift/api/route/v1`: Manages OpenShift Routes (legacy or platform-specific pattern)
- `openshift/api/config/v1`: Accesses OpenShift cluster configuration
- `gateway.networking.k8s.io`: Manages Gateway API resources (modern pattern)
...
**Ingress Pattern Evolution** (CRITICAL TO UNDERSTAND):
Controllers may create different ingress resources depending on platform version or configuration. Read controller code to identify actual behavior:

**Modern Gateway API Pattern**:
- Creates `HTTPRoute` CRs (Gateway API) for main service traffic
- May use auth proxy sidecars (e.g., `kube-rbac-proxy`, `oauth-proxy`)
- Look for: `gateway.networking.k8s.io/v1beta1` or `v1`, `HTTPRoute`, `GRPCRoute`

**Platform-Specific Ingress (e.g., OpenShift Routes)**:
- Creates platform-specific ingress resources (e.g., OpenShift `Route` CRs)
- May use platform-specific auth proxies
- Look for: `route.openshift.io`, platform-specific ingress types

**Hybrid/Migration Patterns**:
Controllers may create BOTH modern and legacy ingress resources for:
- **Redirect resources**: Legacy ingress pointing to redirect services for backwards compatibility
- **OAuth callback endpoints**: Platform-specific resources needed for OAuth flows
- **Alternative ingress modes**: Configurable ingress types based on platform capabilities
- Do NOT skip or dismiss any ingress-creating code. Document ALL resources the controller creates, regardless of whether they use "new" or "legacy" patterns. The distinction is what traffic the resource handles, not whether it's considered legacy.
...
**Examples of controller-created ingress**:
- Notebook/workload controllers: May create HTTPRoutes + auth proxy sidecars per instance
- Platform gateway controllers: May deploy Gateway infrastructure that other controllers reference
- Redirect controllers: May create redirect services + legacy ingress resources for backwards compatibility
- Document ALL resources the controller creates — both modern (HTTPRoute) AND legacy (platform-specific), noting the purpose of each
...
- Common patterns:
  - Platform operator deploys Gateway infrastructure + routing rules + auth components
  - Component controllers create HTTPRoute CRs that reference a parent Gateway
  - Component controllers inject auth proxy sidecars (e.g., kube-rbac-proxy, oauth-proxy)
...
- Common auth proxy patterns:
  - `kube-rbac-proxy`, `oauth-proxy`, or custom auth proxies for authentication
  - Typically on port 8443/TCP or similar
  - Fronts main application container
  - Enforces authentication/authorization before proxying to application
- Document in Network Architecture → Services section
- Example: "auth-proxy sidecar (8443/TCP) → application container (8080/TCP)"
...
- **Version**: [Version]
- **Platform**: [Platform name(s) if applicable]
- **Languages**: [Languages]
...
- For modern platforms: HTTPRoute + auth proxy sidecars (modern pattern)
- For legacy platforms: Platform-specific ingress + auth proxies (legacy pattern)
...
- Example: "Workload Controller | Go Operator | Creates HTTPRoute CRs and auth-proxy sidecars per workload instance"
- Example: "Gateway Controller | Go Operator | Deploys Gateway API infrastructure (Gateway, routing rules, auth proxies)"
...
- Gateway API pattern (modern): Controllers create HTTPRoute CRs + auth proxy sidecars
  - Operator deploys Gateway infrastructure (Gateway API + routing)
  - Component controllers create HTTPRoute CRs referencing parent Gateway
  - Component controllers inject auth proxy sidecars for authentication
- Document type accurately: "HTTPRoute (Gateway API)" vs "Route (platform-specific)" vs "Ingress"
- Include auth components: auth proxy type (e.g., "kube-rbac-proxy", "oauth-proxy")
- Example for workload controller: "Creates HTTPRoute CRs per instance with auth-proxy sidecar authentication"
...
Write the output file. If the prompt specifies an exact output path, write to that path.
Otherwise, create `GENERATED_ARCHITECTURE.md` in the repository root.
