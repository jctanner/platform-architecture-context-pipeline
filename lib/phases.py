...
"""
    ...
    if getattr(args, 'components', None):
        ...
        if patterns_not_matched:
            print(f"ERROR: No components matched pattern(s): {', '.join(patterns_not_matched)}")
            print(f"Available components: {', '.join(sorted(set(j['component_name'] for j in jobs)))}")
            return

        jobs = filtered_jobs
        if len(jobs) == 1:
            print(f"Filtered to single component: {jobs[0]['component_name']}\n")
        else:
            print(f"Filtered to {len(jobs)} component(s): {', '.join(sorted(set(j['component_name'] for j in jobs)))}\n")
    ...
    if successful:
        ...
        if unique_dirs:
            for diagrams_dir in sorted(unique_dirs):
                print(f"Generating PNGs in: {diagrams_dir} (width={args.png_width})")
                try:
                    subprocess.run(
                        [
