...
def get_version_from_makefile(makefile_path: Path) -> Optional[str]:
    ...
    try:
        ...
        if match:
            ...
            # Skip if it contains Makefile constructs like $(shell ...), ${...}, etc.
...
def discover_platforms(checkouts_dir: Path, org_filter: Optional[str] = None, skip_version: bool = False) -> list[Platform]:
    """
    Discover available platform checkouts and their versions.

    Args:
        checkouts_dir: Base checkouts directory
        org_filter: Optional org name to filter to (e.g., 'ansible', 'opendatahub-io')
        skip_version: If True, don't detect version (use empty string)
    """
    platforms = []

    # If org_filter is specified, only process that org
    if org_filter:
        # Look for org directory with or without branch suffix
        for org_dir in sorted(checkouts_dir.glob(f'{org_filter}*')):
            if org_dir.is_dir():
                # For generic orgs (not odh/rhoai), use org name as platform name
                platform_name = org_filter

                # Try to find an operator directory (common patterns)
                operator_dir = None
                for pattern in ['*-operator', 'operator', platform_name]:
                    candidates = list(org_dir.glob(pattern))
                    if candidates:
                        operator_dir = candidates[0]
                        break

                # If no operator found, use the org directory itself for version detection
                if not operator_dir:
                    operator_dir = org_dir

                # Determine version
                version = ""
                if not skip_version:
                    # Extract version from directory name if it has a suffix (e.g., ansible.v2.5)
                    if '.' in org_dir.name and org_dir.name != org_filter:
                        # Has a suffix, extract it
                        parts = org_dir.name.split('.', 1)
                        if len(parts) == 2:
                            version = parts[1]
                    else:
                        # No suffix in directory name, try to detect version
                        version = detect_platform_version(platform_name, org_dir, operator_dir.name if operator_dir != org_dir else '')
                        if not version:
                            version = "unknown"
                ...
                platforms.append(Platform(
                    name=platform_name,
                    version=version,
                    checkout_dir=org_dir,
                    operator_dir=operator_dir
                ))
        return platforms

    # Otherwise, use hardcoded platform discovery (legacy behavior for odh/rhoai)
...
def create_index_readme(output_dir: Path, platform: Platform, components: list[str]):
    ...
    # Build title
    if platform.version:
        title = f"# {platform.name.upper()} {platform.version} - Component Architectures"
    else:
        title = f"# {platform.name.upper()} - Component Architectures"
...
"""
    ...
    if platform.version:
        summary_lines.append(f"- **Version**: {platform.version}")
    summary_lines.extend([
        f"- **Components**: {len(components)}",
        f"- **Source**: {platform.checkout_dir}",
        "",
        "## Using These Files",
        "",
    ...
    if platform.version:
        summary_lines.append(f"/aggregate-platform-architecture --distribution={platform.name} --version={platform.version}")
    else:
        summary_lines.append(f"/aggregate-platform-architecture --distribution={platform.name}")
    ...
    summary_lines.extend([
        ...
        "",
        "```bash",
        f"/generate-architecture-diagrams --architecture=./{components[0] if components else 'COMPONENT'}.md",
        "```",
    ])
...
def collect_architectures(checkouts_dir: Path, output_dir: Path, platform_filter: Optional[str] = None, version_filter: Optional[str] = None, org_filter: Optional[str] = None) -> dict:
    """
    ...
    Args:
        ...
        org_filter: Optional org name to collect from (e.g., 'ansible'). When specified, version detection is skipped.
    ...
    # When using org_filter, skip version detection by default
    skip_version = bool(org_filter)

    # Discover platforms
    platforms = discover_platforms(checkouts_dir, org_filter=org_filter, skip_version=skip_version)
    ...
    for platform in platforms:
        ...
        platform_display = f"{platform.name.upper()}"
        if platform.version:
            platform_display += f" {platform.version}"

        print(f"\nProcessing {platform_display}...")

        # Create output directory
        if platform.version:
            platform_output_dir = output_dir / f"{platform.name}-{platform.version}"
        else:
            platform_output_dir = output_dir / platform.name
    ...
    if summary['platforms']:
        ...
        for platform_info in summary['platforms']:
            if platform_info['version']:
                ...
                print(f"  - {platform_info['name'].upper()} "
                      f"({platform_info['component_count']} components)")
        ...
        for platform_info in summary['platforms']:
            if platform_info['components']:
                ...
                if platform_info['version']:
                    arch_path = f"{output_dir}/{platform_info['name']}-{platform_info['version']}/{first_component}.md"
                else:
                    arch_path = f"{output_dir}/{platform_info['name']}/{first_component}.md"
